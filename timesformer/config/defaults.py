...
# 'cls_only', 'dist_only', 'cls_plus_dist', 'cls_plus_dist_sm'
# Typically cls_plus_dist does better, the softmax hurts performance
_C.TEST.EVAL_PROTOCOL = 'cls_only'
...
_C.EXPERIMENTAL.DEBUG.NOISE_LEVEL = 0.0 # If > 0, will add noise to the 2D keypoints from a normal distribution with std = NOISE_LEVEL

# These will control the folder that contains each datasamples Hyperformer features
_C.EXPERIMENTAL.HYPERFORMER_FEATURES_PATH = ''

# These will control the layers to insert each distillation type in the vit_skeleton_distilled_feature_cls_v2 model
_C.EXPERIMENTAL.LAYERS_POS_GLOBAL_ALIGN_3DSIM = []
_C.EXPERIMENTAL.LAYERS_POS_TEMPORAL_ALIGN_3DSIM = []
_C.EXPERIMENTAL.LAYERS_POS_JOINT_ALIGN_3DSIM = []

_C.EXPERIMENTAL.LAYERS_2DSIM = []


# -----------------------------------------------------------------------------
# Loss scaling options
# -----------------------------------------------------------------------------
_C.LOSS_SCALE = CfgNode()
_C.LOSS_SCALE.CLS_SCALE = 1.0
_C.LOSS_SCALE.SCALE_3DSIM_ALIGN = 1.0
_C.LOSS_SCALE.SCALE_3DSIM_CLS = 1.0
_C.LOSS_SCALE.SCALE_2DSIM = 1.6
...
_C.DATA.UNIFORM_SAMPLING = False # will do uniform sampling (same as MMUniformSampleFrames) for train and test
