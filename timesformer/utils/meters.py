    ...
    def finalize_metrics(self, ks=(1, 5), eval_protocol=None):
        ...
        if self.multi_label:
            ...
            if eval_protocol is not None:
                logger.info(f'Saving test set confusion matrix to: {self.cfg.OUTPUT_DIR}/testset_confusion_matrix_{eval_protocol}.npy')
                np.save(f'{self.cfg.OUTPUT_DIR}/testset_confusion_matrix_{eval_protocol}.npy', conf_matrix)
            else:
